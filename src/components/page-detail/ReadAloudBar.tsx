 ...
 * being spoken.
 *
 * The voice comes from speech.ts (Kokoro, with the browser's voice as the
 * fallback); the rest is platform APIs: Intl.Segmenter for the sentence
...
function readPrefs(): { voiceId: KokoroVoice; rate: number } {
  try {
    const raw = localStorage.getItem(PREFS_KEY);
    if (raw) {
      const saved = JSON.parse(raw) as { voiceId?: string; rate?: number };
      return {
        // A system voice saved before Kokoro is no longer on the menu.
        voiceId: isKokoroVoice(saved.voiceId) ? saved.voiceId : DEFAULT_VOICE,
  ...
  return { voiceId: DEFAULT_VOICE, rate: 1 };
}
...
export function ReadAloudBar({
  ...
  const [voiceId, setVoiceId] = useState<KokoroVoice>(initialPrefs.voiceId);
    ...
    >
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <select
          value={voiceId}
          onChange={(e) => {
            if (isKokoroVoice(e.target.value)) setVoiceId(e.target.value);
          }}
          aria-label="Voice"
