...
export function RecordingPanel({ pageId }: { pageId: string }) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-zinc-200/80 bg-white p-6">
      ...
      {(inSession || (status === "stopped" && liveTranscript)) && (
        <div className="flex flex-col gap-2 border-t border-zinc-100 pt-4">
          ...
          <div className="max-h-40 overflow-y-auto rounded-lg bg-zinc-50 p-3 text-[13px] leading-6 text-zinc-600">
            {liveTranscript || <span className="text-zinc-400">Listening… the transcript appears here as the lecture goes on.</span>}
            <div ref={liveEndRef} />
          </div>
          ...
          {explanation && (
            <div className="rounded-lg border border-brand-border bg-brand-soft/50 p-3">
              ...
              <p className="text-[13px] leading-6 text-zinc-700">{explanation}</p>
            </div>
          )}
        </div>
      )}
