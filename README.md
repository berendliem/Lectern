...
**Talk it through (live).** On an active interview session, *Talk it through* turns it into a spoken one, like a voice call: the tutor speaks, you answer out loud, a short pause ends your turn, and talking over the tutor cuts it off. When you're wrong, it names the slip and walks through a worked example. The example comes from your course material when the session has some; otherwise the tutor says the example is made up. A Viva then gives you one variant to retry, a Protégé asks you to explain it again, and in a Debate the next speaker corrects your point or concedes it. Captions highlight each word as it is spoken (CC toggles them). When the session ends you get the transcript plus a *What to fix* sheet you can copy or download as Markdown. Your answers are transcribed locally (Whisper or mac-speech). If local transcription fails, Chrome's text is used instead and marked "browser transcript". Chrome produces that text with Google's speech service, which hears your audio whenever the session is listening. The tutor speaks in your read-aloud voice, and if that is a network voice, its provider synthesises the tutor's replies. Debaters use two fixed voices.
...
With [onq-mcp](https://github.com/berendliem/Qmcp) configured as the `onq` server in
