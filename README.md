...
- **Study-plan agent**: a per-course agentic run spawned when you hit **Generate study plan** on a course page. Lectern starts the [`claude` CLI](https://claude.com/claude-code) with one MCP server of its own (`scripts/lectern-mcp.ts`) that exposes that course's lectures, syllabus coverage, and due cards. The agent reads them, writes a week's plan, and records the action items and review blocks it recommends. It runs on your existing Claude Code credential rather than an API key, and it can only see the one course it was pointed at.
